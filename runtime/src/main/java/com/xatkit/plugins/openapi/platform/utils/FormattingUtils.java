...
public class FormattingUtils {
	...
	public static String formatOperation(Operation o) {
		return new StringBuilder().append((nonNull(o.getOperationId()) && !o.getOperationId().equals("")) ? "`" + o.getOperationId() + "`: " :"").append("`"+o.getHTTPMethod()).append(" ").append(((Path)o.eContainer()).getRelativePath()+"`, ").append((nonNull(o.getSummary()) && !o.getSummary().equals("")) ? "(" + o.getSummary() + ")" :"").toString();
	}
	...
	public static String formatResponse(Response response) {
		...
		if (response.getSchema() != null) {
			...
			result.append(": no schema defined");
	...
	public static String formatParameter(Parameter parameter) {
		...
		case BODY:
			result.append(" (body parameter)");
			break;
		case FORM_DATA:
			result.append(" (form parameter)");
			break;
		case HEADER:
			result.append(" (header parameter)");
			break;
		case PATH:
			result.append(" (path parameter)");
			break;
		case QUERY:
